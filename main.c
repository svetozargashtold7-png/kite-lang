...
static int run_source(const char *src, int verbose, const char *filepath) {
    ...
    if (filepath) {
        ...
        if (last_slash) {
            ...
            if (dlen < (int)sizeof(ip->script_dir)-1) {
                strncpy(ip->script_dir, filepath, (size_t)dlen);
                ip->script_dir[dlen] = '\0';
            }
        } else {
            strcpy(ip->script_dir, ".");
        }
    } else {
        strcpy(ip->script_dir, ".");
    }
...
static void repl(void) {
    ...
    while (1) {
        ...
        if (in_block==0 && strlen(multi)>0) {
            ...
            Node *ast=parse(l);
            if (ast) {
                Interp *ip=interp_new(); strcpy(ip->script_dir,".");
                eval(ip,ast,env);
...
int main(int argc, char *argv[]) {
    ...
    if (!src) return 1;
    int ret = run_source(src, verbose, filename);
    free(src); return ret;
}
